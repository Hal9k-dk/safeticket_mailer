...
class SafeTicket:
  ...
  def single_event(self, Id: event: str, cash: int):
    data = {
        'version': self._version,
        'user': self._user,
        'event': event,
    ...
    return requests.post('https://{}/api/stats/{}'.format(self._host, Id), data)

  def single_order(self, Id: str):
    data = {
        'version': self._version,
        'user': self._user,
        'orderid': Id,
        'sha': self._calc_sha([Id])}
