...
def main():
    ...
    for row in _csv_reader:
        l: List[str] = ticket_types[row['Billettype']]
        l.append(row)
        l.append(row)
    ...
    for index, union in enumerate(CONFIG.unions):
        ...
        for ticket_type_name in union.ticket_type_names:
            try:
                ...
                print(
                    "ERROR: The fields {} doesn't exist, check the variable 'ticket_fields' in the config.py file".format(
                        e))
                sys.exit(1)
        ...
        if args.send_emails:
            if datetime.strptime(event['settledate'], "%d.%m.%Y").date() + timedelta(
                    days=CONFIG.extra_days_to_send_emails) >= date.today():
